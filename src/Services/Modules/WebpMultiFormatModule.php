...
/**
 * WebpMultiFormatModule - WEBP to JPG, PNG, and PDF conversion using ImageMagick
 * 
 * This multi-tool module handles three different output formats from WEBP:
 * - WEBP to JPG: Lossy conversion with background color handling
 * - WEBP to PNG: Lossless conversion preserving transparency
 * - WEBP to PDF: Document conversion maintaining image quality
 */
class WebpMultiFormatModule extends AbstractConversionModule
{
    ...
    public function __construct(string $toFormat = 'jpg')
    {
        ...
        parent::__construct('webp', strtolower($toFormat), 'imagemagick');
    }
    ...
    protected function isToolAvailable(): bool
    {
        ...
        // Check WEBP input support first
        if (!$this->checkWebpInputSupport($convertPath)) {
            return $availability[$cacheKey] = false;
        }
        ...
        Logger::debug("ImageMagick available for WEBP to {$this->toFormat} conversion");
        return $availability[$cacheKey] = true;
    }
    
    /**
     * Execute WEBP conversion to the specified target format
     */
    ...
    /**
     * Convert WEBP to JPG with background color handling
     */
    private function convertToJpg(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick WEBP to JPG conversion', [
            'input' => basename($inputFile),
    ...
    private function convertToPng(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick WEBP to PNG conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
    ...
    /**
     * Convert WEBP to PDF maintaining image quality
     */
    private function convertToPdf(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick WEBP to PDF conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
            'quality' => $quality
        ]);
    ...
    /**
     * Check if ImageMagick can read WEBP files (input support)
     */
    private function checkWebpInputSupport(string $convertPath): bool
    {
        $result = $this->executeCommand($convertPath . ' -list format | grep -i webp', 10);
        
        if (!$result['success'] || empty(trim($result['output']))) {
            Logger::warning('ImageMagick WEBP input support not detected', [
                'suggestion' => 'Install libwebp-dev and recompile ImageMagick for WEBP support'
            ]);
            return false;
        }
        
        // Check if WEBP format supports reading (should show 'r' in format string)
        $formatOutput = $result['output'];
        if (strpos($formatOutput, 'r') === false && strpos($formatOutput, 'rw') === false) {
            Logger::warning('ImageMagick WEBP read support not available');
            return false;
        }
        
        Logger::debug('WEBP input support confirmed');
        return true;
    }
    ...
    {
        return strtolower($fromFormat) === 'webp' && 
