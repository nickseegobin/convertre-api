...
/**
 * GifMultiFormatModule - GIF to JPG, PNG, and PDF conversion using ImageMagick
 * 
 * This multi-tool module handles three different output formats from GIF:
 * - GIF to JPG: Lossy conversion with background color handling (for transparency)
 * - GIF to PNG: Lossless conversion preserving transparency and animation frame
 * - GIF to PDF: Document conversion maintaining image quality
 */
class GifMultiFormatModule extends AbstractConversionModule
{
    ...
    public function __construct(string $toFormat = 'jpg')
    {
        ...
        parent::__construct('gif', strtolower($toFormat), 'imagemagick');
    }
    ...
    protected function isToolAvailable(): bool
    {
        ...
        Logger::debug("ImageMagick available for GIF to {$this->toFormat} conversion");
        return $availability[$cacheKey] = true;
    }
    
    /**
     * Execute GIF conversion to the specified target format
     */
    ...
    /**
     * Convert GIF to JPG with background color handling
     */
    private function convertToJpg(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick GIF to JPG conversion', [
            'input' => basename($inputFile),
    ...
    /**
     * Convert GIF to PNG preserving transparency
     */
    private function convertToPng(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick GIF to PNG conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
    ...
    /**
     * Convert GIF to PDF maintaining image quality
     */
    private function convertToPdf(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick GIF to PDF conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
            'quality' => $quality
        ]);
    ...
    private function buildJpgCommand(string $convertPath, string $input, string $output, int $quality): string
    {
        ...
        // JPG-specific options (handle transparency and animation)
        $options = [
            ...
            '[0]',                       // Take first frame (for animated GIFs)
        ...
        return sprintf(
            '%s %s %s %s',
            $convertPath,
            $input . '[0]',              // Specify first frame in input
            implode(' ', array_slice($options, 0, -2)), // All options except [0] and quality
    ...
    private function buildPngCommand(string $convertPath, string $input, string $output, int $compression): string
    {
        ...
        // PNG-specific optimization options (preserve transparency)
        $options = [
            '-strip',                    // Remove metadata
            '-auto-orient',              // Fix orientation
            '-colorspace sRGB',          // Consistent color space  
        ...
        return sprintf(
            ...
            $convertPath,
            $input . '[0]',              // Specify first frame in input
            implode(' ', array_slice($options, 0, -3)), // All options except [0], compress, and quality
            '-compress Zip -quality ' . $compression . ' ' . $output
        );
    }
    ...
    private function buildPdfCommand(string $convertPath, string $input, string $output, int $quality): string
    {
        ...
        return sprintf(
            ...
            $convertPath,
            $input . '[0]',              // Specify first frame in input
            implode(' ', array_slice($options, 0, -6)), // All options except the last 6
            '-quality ' . $quality . ' -density 300 -compress JPEG -page A4 -gravity center ' . $output
        );
    }
    ...
    {
        return strtolower($fromFormat) === 'gif' && 
