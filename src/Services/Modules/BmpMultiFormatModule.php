...
/**
 * BmpMultiFormatModule - BMP to JPG, PNG, and PDF conversion using ImageMagick
 * 
 * This multi-tool module handles three different output formats from BMP:
 * - BMP to JPG: Lossy conversion with optimized quality
 * - BMP to PNG: Lossless conversion with compression
 * - BMP to PDF: Document conversion maintaining image quality
 */
class BmpMultiFormatModule extends AbstractConversionModule
{
    ...
    public function __construct(string $toFormat = 'jpg')
    {
        ...
        parent::__construct('bmp', strtolower($toFormat), 'imagemagick');
    }
    ...
    protected function isToolAvailable(): bool
    {
        ...
        Logger::debug("ImageMagick available for BMP to {$this->toFormat} conversion");
        return $availability[$cacheKey] = true;
    }
    
    /**
     * Execute BMP conversion to the specified target format
     */
    ...
    /**
     * Convert BMP to JPG with optimized quality
     */
    private function convertToJpg(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick BMP to JPG conversion', [
            'input' => basename($inputFile),
    ...
    private function convertToPng(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick BMP to PNG conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
    ...
    /**
     * Convert BMP to PDF maintaining image quality
     */
    private function convertToPdf(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick BMP to PDF conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
            'quality' => $quality
        ]);
    ...
    private function buildJpgCommand(string $convertPath, string $input, string $output, int $quality): string
    {
        ...
        // JPG-specific options (BMP is always opaque, no transparency handling needed)
        $options = [
    ...
    {
        return strtolower($fromFormat) === 'bmp' && 
