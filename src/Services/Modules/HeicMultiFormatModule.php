...
/**
 * HeicMultiFormatModule - HEIC to JPG, PNG, and PDF conversion using ImageMagick
 * 
 * This multi-tool module handles three different output formats from HEIC:
 * - HEIC to JPG: High-quality lossy conversion maintaining color accuracy
 * - HEIC to PNG: Lossless conversion for maximum quality
 * - HEIC to PDF: Document conversion maintaining image quality
 */
class HeicMultiFormatModule extends AbstractConversionModule
{
    ...
    public function __construct(string $toFormat = 'jpg')
    {
        ...
        parent::__construct('heic', strtolower($toFormat), 'imagemagick');
    }
    ...
    protected function isToolAvailable(): bool
    {
        ...
        // Check HEIC input support first (critical for Apple format)
        if (!$this->checkHeicInputSupport($convertPath)) {
            return $availability[$cacheKey] = false;
        }
        ...
        Logger::debug("ImageMagick available for HEIC to {$this->toFormat} conversion");
        return $availability[$cacheKey] = true;
    }
    
    /**
     * Execute HEIC conversion to the specified target format
     */
    ...
    /**
     * Convert HEIC to JPG with optimal quality
     */
    private function convertToJpg(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick HEIC to JPG conversion', [
            'input' => basename($inputFile),
    ...
    private function convertToPng(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick HEIC to PNG conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
    ...
    /**
     * Convert HEIC to PDF maintaining image quality
     */
    private function convertToPdf(string $convertPath, string $inputFile, string $outputFile): bool
    {
        ...
        Logger::debug('ImageMagick HEIC to PDF conversion', [
            'input' => basename($inputFile),
            'output' => basename($outputFile),
            'quality' => $quality
        ]);
    ...
    private function buildJpgCommand(string $convertPath, string $input, string $output, int $quality): string
    {
        ...
        // JPG-specific options optimized for HEIC conversion
        $options = [
            '-strip',                    // Remove metadata
            '-auto-orient',              // Fix orientation (important for HEIC)
    ...
    /**
     * Check if ImageMagick can read HEIC files (input support)
     */
    private function checkHeicInputSupport(string $convertPath): bool
    {
        ...
        if (!$result['success'] || empty(trim($result['output']))) {
            ...
            $heifResult = $this->executeCommand($convertPath . ' -list format | grep -i heif', 10);
            
            if (!$heifResult['success'] || empty(trim($heifResult['output']))) {
                Logger::warning('ImageMagick HEIC/HEIF input support not detected', [
                    'suggestion' => 'Install libheif-dev and recompile ImageMagick for HEIC support'
                ]);
                return false;
            }
            
            $formatOutput = $heifResult['output'];
        } else {
            $formatOutput = $result['output'];
        }
        
        // Check if HEIC format supports reading (should show 'r' in format string)
        if (strpos($formatOutput, 'r') === false && strpos($formatOutput, 'rw') === false) {
            Logger::warning('ImageMagick HEIC read support not available');
            return false;
        }
        
        Logger::debug('HEIC input support confirmed');
        return true;
    }
    ...
    {
        return strtolower($fromFormat) === 'heic' && 
